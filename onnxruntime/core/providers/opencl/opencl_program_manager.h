...
class OpenCLProgramManager {
 public:
  ...
  void SetLocalSizeUsingGlobal(const NDRange& global, NDRange local);
  std::optional<const NDRange> GetLocalSizeUsingGlobal(const NDRange& global);
 private:
  struct ProgramMeta {
    ProgramKey key;
    int32_t rc;
  ...
  std::unordered_map<NDRange, NDRange, absl::Hash<NDRange>> global_map_to_local_size;
};
